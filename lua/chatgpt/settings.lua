...
M.read_config = function()
  local file = io.open((package.config:sub(1,1) == "/" and os.getenv("HOME") or os.getenv("UserProfile")) .. "/" .. ".chatgpt-" .. M.type .. "-params.json", "rb")
  if not file then
    return nil
  end
...
M.write_config = function(config)
    local file, err = io.open((package.config:sub(1,1) == "/" and os.getenv("HOME") or os.getenv("UserProfile")) .. "/" .. ".chatgpt-" .. M.type .. "-params.json", "w")
  if file ~= nil then
